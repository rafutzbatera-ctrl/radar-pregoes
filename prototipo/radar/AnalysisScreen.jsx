...
function AnalysisScreen({ pregao, estado, setCusto, setMatch, setHabil, voltar, scrollRef, meterVariant, config, setRegime }) {
  const { reduzido, estatico } = React.useContext(MotionCtx);
  const a = analisar(pregao, estado);
  ...
  const conteudo = (
    <motion.div className="screen-inner" variants={pai} initial={estatico ? false : "hidden"} animate="show">
      <div ref={cabRef} style={{ willChange: "transform" }}>
        <motion.div variants={entrada}>
          ...
          <div className="cab-info">
            <div className="silk">{pregao.modalidade} · {pregao.amparo} · {pregao.situacao || pregao.status}</div>
            <h1>{pregao.titulo}</h1>
            <div className="cab-orgao">{pregao.orgao} · {pregao.municipio}/{pregao.uf}</div>
            ...
            <a className="link-pncp" href={linkPncp(pregao)} target="_blank" rel="noopener noreferrer">
              {Ico.externo} Ver no PNCP · {pregao.numeroControle}
            </a>
        ...
        <motion.section className="hero" variants={entrada} aria-label="Veredito de viabilidade">
          <div className="hero-top">
            <div>
              ...
              <div className="hero-lucro"><NumBRL value={a.lucroTotal} /></div>
            </div>
            <div>
              <div className="silk">Margem média</div>
              <div className="hero-margem"><NumPct value={a.margemAgregada} /></div>
            </div>
            <div className="hero-veredito">
              <div className="silk">Veredito</div>
              <div className={"hero-veredito-palavra " + a.veredito} aria-live="polite">{VEREDITO_TXT[a.veredito]}</div>
            </div>
          </div>
          <Medidor valor={a.margemAgregada} variante={meterVariant} delay={reduzido ? 0 : 0.35} />
        ...
        <motion.div className="resumo" variants={entrada}>
          <Resumo k="Valor total estimado" v={fmtBRL(pregao.valorTotal)} />
          ...
          <Resumo k="Habilitação" v={habil.length ? pendentesHabil + " pendentes" : "não sincronizada"} sub={habil.length ? "de " + habil.length + " requisitos" : undefined} />
      ...
      <motion.div variants={entrada} key={aba} className="aba-painel">
        {aba === "itens" && (
          <ItensTab a={a} pregao={pregao} abrir={abrirItem} setCusto={setCusto} setMatch={setMatch} estatico={estatico} reduzido={reduzido} itemAberto={itemAberto} />
        )}
        {aba === "habilitacao" && (
  ...
  return (
    <React.Fragment>
      ...
      <DetailPanel item={aberto} pregao={pregao} fechar={fecharItem} setCusto={setCusto} setMatch={setMatch} config={config} />
    </React.Fragment>
  );
}
...
/* ============ ABA ITENS ============ */
function ItensTab({ a, pregao, abrir, setCusto, setMatch, estatico, reduzido, itemAberto }) {
    ...
    <div className="tbl-wrap">
      ...
      <div className="mod tbl" role="table" aria-label="Itens do pregão">
        ...
        <motion.div variants={tbl} initial={estatico ? false : "hidden"} animate="show" role="rowgroup">
          {a.itens.map((it) => (
            <LinhaItem key={it.n} item={it} variants={linha} ativa={itemAberto === it.n} abrir={abrir} setCusto={setCusto} setMatch={setMatch} />
          ))}
...
function LinhaItem({ item, variants, ativa, abrir, setCusto, setMatch }) {
  const ref = React.useRef(null);
  const sigiloso = item.status === "sigiloso";
  const semConta = !item.coberto; // sugerido, fora ou sigiloso
    ...
    <motion.div className="tbl-item" variants={variants}>
      ...
      >
        ...
        <div className="c-unit" role="cell">
          ...
          {sigiloso ? <span className="sigilo-tag">sigiloso</span> : fmtBRL(item.unit)}
        </div>
        <div className="c-custo" role="cell">
          <span className="m-label mobile-only">Seu custo unit.</span>
          {item.produto && !sigiloso ? <CostInput item={item} setCusto={setCusto} /> : <span style={{ paddingRight: "8px", color: "var(--silk)" }}>—</span>}
        </div>
      ...
      {!sigiloso && (item.status === "sugerido" || item.status === "fora") && (
        <MatchBar item={item} setMatch={setMatch} />
      )}
    </motion.div>
  );
}
...
function MatchBar({ item, setMatch }) {
  const [trocando, setTrocando] = React.useState(false);
  const cat = window.RADAR_DATA.catalogo;
    ...
    return (
      <div className="match-bar sem">
        ...
        {!trocando ? (
          ...
          <SeletorProduto cat={cat} onPick={(cod) => { setMatch(item.n, { cod, score: null, confirmado: true }); setTrocando(false); }} onCancel={() => setTrocando(false)} />
        )}
      </div>
    );
  }
    ...
    <div className="match-bar sugerido">
      ...
      {!trocando ? (
        <div className="match-acoes">
          <button type="button" className="match-btn ok" onClick={() => setMatch(item.n, { ...m, confirmado: true })}>{Ico.check} Confirmar</button>
          <button type="button" className="match-btn trocar" onClick={() => setTrocando(true)}>Trocar</button>
          <button type="button" className="match-btn nao" onClick={() => setMatch(item.n, null)}>{Ico.fechar} Recusar</button>
        </div>
      ) : (
        <SeletorProduto cat={cat} atual={m.cod} onPick={(cod) => { setMatch(item.n, { cod, score: null, confirmado: true }); setTrocando(false); }} onCancel={() => setTrocando(false)} />
      )}
    </div>
  );
}
...
/* ---------- célula de custo editável ---------- */
function CostInput({ item, setCusto }) {
  const [editando, setEditando] = React.useState(false);
  const [rascunho, setRascunho] = React.useState("");
  const fmt = (v) => v.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const valor = editando ? rascunho : fmt(item.custo);
  const confirmar = () => {
    const v = parseBRL(rascunho);
    if (v != null && v >= 0) setCusto(item.n, Math.round(v * 100) / 100);
    setEditando(false);
  };
  return (
    <input
      className="custo-input" type="text" inputMode="decimal" value={valor}
      aria-label={"Seu custo unitário do item " + item.n + " — editável"}
      onFocus={(e) => { setRascunho(fmt(item.custo)); setEditando(true); requestAnimationFrame(() => e.target.select()); }}
      onChange={(e) => { setRascunho(e.target.value); const v = parseBRL(e.target.value); if (v != null && v >= 0) setCusto(item.n, Math.round(v * 100) / 100); }}
      onBlur={confirmar}
...
Object.assign(window, { AnalysisScreen, STATUS_TXT, ItensTab, LinhaItem, MatchBar, SeletorProduto, CostInput });
